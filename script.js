(() => {
    ...
    const VISIBILITY = window.__YT_BATCH_PUBLISHER_VISIBILITY__ || 'Unlisted'; // 'Public' / 'Private' / 'Unlisted'
    // -----------------------------------------------------------------
    ...
    function click(element) {
        ...
        try {
            ...
            element.click();
        } catch (_) { try { element.click(); } catch (e) { } }
        debugLog(element, 'clicked');
    }

    function autoClosePublishPopupInstant(onClose) {
        const observer = new MutationObserver(() => {
            const popup = document.querySelector("ytcp-video-share-dialog, ytcp-dialog.ytcp-video-share-dialog");
            if (!popup) return;

            const closeBtn = popup.querySelector('[aria-label="Close"], tp-yt-paper-icon-button, tp-yt-icon-button, iron-icon[icon="close"]');
            if (closeBtn) {
                click(closeBtn);
                debugLog("🔥 Instant popup close triggered");
                observer.disconnect();
                if (onClose) onClose();
            }
        });
    ...
    async function tryClosePublishPopupFallback(timeoutMs = 3000) {
        const start = Date.now();

        async function attempt() {
            const dialogs = [
                'ytcp-video-share-dialog',
                'ytcp-dialog.ytcp-video-share-dialog',
            ...
            for (const sel of dialogs) {
                ...
                const cand = box.querySelector('[aria-label="Close"], tp-yt-paper-icon-button, tp-yt-icon-button, iron-icon[icon="close"]');
                if (cand) {
                    click(cand);
                    await sleep(100);
                    if (!document.contains(box)) return true;
                }
            }
        ...
        while (Date.now() - start < timeoutMs) {
            if (await attempt()) {
                debugLog("✔ Popup closed via fallback");
                return true;
            }
            await sleep(120);
        }
    ...
    class SuccessDialog {
        ...
        async close() {
            return new Promise(async resolve => {
                ...
                setTimeout(async () => {
                    if (done) return;
                    await tryClosePublishPopupFallback();
                    resolve(true);
                }, 400);
            });
        }
